...
// Disk storage — session uploads (must persist for listing/download)
...
// ── Download inbox / assembled chunk file ────────────────
...
router.post("/os-transfer/send", osUpload.array("files", 50), (req, res) => {
    ...
    req.files.forEach(file => {
        addToOsQueue(code, { originalName: file.originalname, storedName: file.filename, size: file.size });
    });

    res.json({ message: `${req.files.length} file(s) queued successfully`, code, fileCount: queue.files.length });
});
...
router.get("/os-transfer/claim/:code", async (req, res) => {
    ...
    if (!queue.files.length)  return res.status(400).json({ error: "No files in queue yet" });
    ...
    queue.files.forEach(file => {
        const filePath = path.join(osDir, file.storedName);
        if (fs.existsSync(filePath)) archive.file(filePath, { name: file.originalName });
    });
    archive.finalize();
    markOsQueueClaimed(code);
});
